...
async function testDeadlineExtension() {
  try {
    ...
    // First, get current season data
    console.log('\n1. Getting current season data...');
    const currentResponse = await fetch('http://localhost:8080/api/seasons/current/Singles%20Test');
    ...
    const updateResponse = await fetch('http://localhost:8080/admin/update-schedule', {
      ...
      body: JSON.stringify({ division: 'Singles Test' })
    });
    ...
    const updatedResponse = await fetch('http://localhost:8080/api/seasons/current/Singles%20Test');
