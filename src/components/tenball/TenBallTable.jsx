...
const TenBallTable = ({ 
  ...
  const BALL_SIZE = 8;
  const POCKET_SIZE = 18; // visual/detection radius
  ...
  const updateTrajectories = useCallback((angle, power, english) => {
    ...
    // Predict cue ball trajectory (pre-collision)
    let cueTrajectory = predictTrajectory(cueBall.x, cueBall.y, angle, power, english);
    ...
    // If a collision is predicted, approximate cue-ball deflection after first contact
    if (collisions.length > 0) {
      const firstCollision = collisions[0];
      const hitBall = gameState.balls.find(b => b.id === firstCollision.ballId);
      if (hitBall) {
        // Compute cue ball velocity at impact (match executeShot/predictTrajectory units)
        const baseSpeed = power * 18;
        const cueVx = Math.cos(angle) * baseSpeed + english.x * baseSpeed * 0.3;
        const cueVy = Math.sin(angle) * baseSpeed + english.y * baseSpeed * 0.3;

        // Collision normal from collision point to ball center
        const ndx = hitBall.x - firstCollision.point.x;
        const ndy = hitBall.y - firstCollision.point.y;
        const nd = Math.hypot(ndx, ndy) || 1;
        const nx = ndx / nd;
        const ny = ndy / nd;

        // Transfer along normal; equal-mass elastic approximation
        const normalVelocity = cueVx * nx + cueVy * ny;
        const postCueVx = cueVx - normalVelocity * nx;
        const postCueVy = cueVy - normalVelocity * ny;

        const postSpeed = Math.hypot(postCueVx, postCueVy);
        if (postSpeed > 0.01) {
          const postAngle = Math.atan2(postCueVy, postCueVx);
          const postPower = Math.min(postSpeed / 18, 1.0);
          const postCueTrajectory = predictTrajectory(
            firstCollision.point.x,
            firstCollision.point.y,
            postAngle,
            postPower,
            { x: 0, y: 0 }
          );

          // Splice cue trajectory at impact step and append post-collision prediction
          cueTrajectory = cueTrajectory.slice(0, Math.max(0, firstCollision.step + 1)).concat(postCueTrajectory);
        }
      }
    }

    // Add cue ball trajectory (possibly spliced with post-collision segment)
    trajectories.push({
      ballId: 'cue',
  ...
  // Check if ball is in pocket (use felt-corner/midpoint centers)
  const isInPocket = useCallback((ball) => {
    ...
    return pockets.some(pocket => {
      ...
      const distance = Math.sqrt(dx * dx + dy * dy);
      // Slightly more forgiving than visual radius so balls drop reliably
      return distance < (POCKET_SIZE + BALL_SIZE * 0.5);
    });
  }, []);
  ...
  useEffect(() => {
    ...
    ctx.clearRect(0, 0, TABLE_WIDTH, TABLE_HEIGHT);
       ...
       ctx.drawImage(tableImageCache.current, 0, 0, TABLE_WIDTH, TABLE_HEIGHT);
        // Draw pockets (aligned to felt corners/midpoints)
        const FELT_LEFT = 30.0;
        const FELT_RIGHT = 570.77;
        const FELT_TOP = 24.5;
        const FELT_BOTTOM = 270.18;
  ...
  const highlightTutorialElements = (ctx) => {
    ...
    switch (currentStep) {
      ...
      case 2:
        ...
        break;
      case 3:
        const FELT_LEFT2 = 30.0;
        const FELT_RIGHT2 = 570.77;
        const FELT_TOP2 = 24.5;
        const FELT_BOTTOM2 = 270.18;
        const midX2 = (FELT_LEFT2 + FELT_RIGHT2) / 2;
        const pockets = [
          { x: FELT_LEFT2, y: FELT_TOP2 },
          { x: midX2, y: FELT_TOP2 },
          { x: FELT_RIGHT2, y: FELT_TOP2 },
          { x: FELT_LEFT2, y: FELT_BOTTOM2 },
          { x: midX2, y: FELT_BOTTOM2 },
          { x: FELT_RIGHT2, y: FELT_BOTTOM2 }
        ];
        pockets.forEach(pocket => {
          ctx.beginPath();
